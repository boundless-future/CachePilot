...
- [ ] 实现实际分配计数消融，避免异步回载恢复后的重复计数；若不足再研究分配前需求预算。
...
先完成 P0 → P1 → P2，再决定 P3。初始基线表明压力负载中立即卸载优于默认延迟卸载，而小工作集原生路径更快。9 月 26 日已确认观测到的 KV 传输内容一致，并完成无日志计时和 allocator 诊断。下一步优先验证实际分配信号与提前需求预算，输出差异仍待独立区分编译/CUDA Graph；两点 horizon 试验不代替完整参数扫描。尚不需要更贵的 GPU 或完整 SWE-bench。
